  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "kaesra-transparent")
        draft.active = typeof active === "string" ? active : "kaesra-transparent"
        draft.ready = false
      }),
    )
    ...
    function syncCustomThemes() {
      return themes
        ...
        .catch(() => setStore("active", "kaesra-transparent"))
    }
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "kaesra-transparent")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes["kaesra-transparent"] || store.themes.opencode, store.mode)
    })
